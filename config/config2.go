...
func SaveConfig() error {
	...
	configFile, err := os.OpenFile(state.cfgPath, os.O_CREATE | os.O_TRUNC, os.ModePerm)
	if err != nil {
		return err
	}
...
func (_ Config2) GetAccountEmail() string {
	state.mu.RLock()
	defer state.mu.RUnlock()
	return state.inner.Account.Email
}

func (_ Config2) GetAccountPassword() string {
	state.mu.RLock()
	defer state.mu.RUnlock()
	return state.inner.Account.Password
}

func (_ Config2) GetAccountCompanyId() int {
	state.mu.RLock()
	defer state.mu.RUnlock()
	return state.inner.Account.Company
}

func (_ Config2) SetAccountEmail(email string) error {
	state.mu.Lock()
	defer state.mu.Unlock()
...
func (_ Config2) SetAccountPassword(password string) error {
	state.mu.Lock()
	defer state.mu.Unlock()
	if state.loadedFromEnv {
		return fmt.Errorf("could not set config value %s to %q config was loaded from env",
...
func (_ Config2) SetAccountCompanyId(id int) error {
	state.mu.Lock()
	defer state.mu.Unlock()
	if state.loadedFromEnv {
		return fmt.Errorf("could not set config value %s to %q config was loaded from env",
